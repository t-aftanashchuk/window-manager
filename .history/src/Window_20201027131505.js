...
export class Window extends Events {
    ...
    _downTitlebar(e) {
        ...
        const up = Math.abs(event.pageY - this.y) < 3;
        const down = Math.abs(event.pageY - this.y + this.height) < 3;
        const left = Math.abs(event.pageX - this.x) < 3;
        const right = Math.abs(event.pageX - this.x + this.width) < 3;

        console.log('_downTitlebar', up, down, left, right)
